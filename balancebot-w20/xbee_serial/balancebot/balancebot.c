...
void balancebot_controller(){
	...
	double position = (left_pos + right_pos) / 2;
	...
	mb_state.theta_ref = doOuterLoop(mb_state.distance_ref, position, control_conf_params);
	...
	if(mb_setpoints.ctl == 2){
		...
		mb_state.psi_ref += mb_setpoints.turn_velocity * DT * 3.0;
	...
	while(1){
		...
		if(rc_dsm_is_new_data()){
			if(rc_dsm_ch_normalized(5) == 0){
				mb_setpoints.ctl = 2; //Idle
			} else if(rc_dsm_ch_normalized(5) == 0.5) {
				mb_setpoints.ctl = 1; //Autonomous
			} else if(rc_dsm_ch_normalized(5) == 1){
				mb_setpoints.ctl = 0; //Manual control
			} else {
...
void* printf_loop(void* ptr){
	...
	while(rc_get_state()!=EXITING){
		...
		if(new_state == RUNNING){
			...
			pthread_mutex_lock(&state_mutex);
			printf("%7.3f  |", mb_state.headingDeltaPWM);
			printf("%7.3f  |", mb_state.psi_ref);
			printf("%7.4f  |", mb_state.left_pos);
